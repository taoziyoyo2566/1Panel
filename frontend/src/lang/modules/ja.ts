...
const message = {
    ...
    xpack: {
        ...
        tamper: {
            tamper: 'サイトの整合性',
            tamperHelper1:
                'ワンクリック展開型のウェブサイトでは、アプリケーションディレクトリの改ざん防止機能を有効にすることをお勧めします。ウェブサイトが正常に動作しない場合やバックアップ/復元が失敗した場合は、まず改ざん防止機能を無効にしてください。',
            tamperHelper2:
                'これにより、除外されていないディレクトリ内の保護されたファイルに対して、読み取り、書き込み、削除、権限、および所有者の変更操作が制限されます。',
            op: '操作',
            create: '作成',
            file: 'ファイル',
            tamperPath: '保護されたディレクトリ',
            ...
            ignoreHelper1: '保護から除外するフォルダ名や特定のパスを追加してください。',
            ignoreHelper2: '特定のフォルダを除外するには、./で始まる相対パスを使用してください。',
            protect: '保護ファイル',
            protectHelper: '1行ごとに入力してください。例：\npng\n./test.css',
            protectHelper1: '保護するファイル名、拡張子、または特定のファイルを指定してください。',
        ...
        alert: {
            ...
            log: 'ログバックアップ',
            snapshot: 'システムスナップショット',
            clamsRulesHelper: 'SMSアラートが必要なウイルススキャンタスク',
