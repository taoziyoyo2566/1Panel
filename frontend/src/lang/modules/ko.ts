...
const message = {
    ...
    xpack: {
        ...
        tamper: {
            tamper: '사이트 무결성',
            tamperHelper1:
                '원클릭 배포 유형의 웹사이트에서는 애플리케이션 디렉터리 방지 조작 기능을 활성화하는 것이 좋습니다. 웹사이트에 문제가 발생하거나 백업/복원이 실패한 경우 먼저 방지 조작 기능을 비활성화하세요.',
            tamperHelper2:
                '보호된 파일이 포함된 디렉터리에서 제외되지 않은 파일의 읽기, 쓰기, 삭제, 권한 및 소유자 수정 작업을 제한합니다.',
            op: '작업',
            create: '생성',
            file: '파일',
            tamperPath: '보호 디렉터리',
            ...
            log: '차단 로그',
            totalProtect: '총 보호 수',
            todayProtect: '오늘의 보호 수',
            addRule: '규칙 추가',
            ignore: '제외 디렉터리',
            ignoreHelper: '한 줄에 하나씩 입력하세요. 예: \ntmp\n./tmp',
            ignoreHelper1: '보호에서 제외할 폴더 이름이나 특정 경로를 추가하세요.',
            ignoreHelper2: './로 시작하는 상대 경로를 사용하여 특정 폴더를 제외하세요.',
            protect: '보호된 파일',
            protectHelper: '한 줄에 하나씩 입력하세요. 예: \npng\n./test.css',
            protectHelper1: '보호할 파일 이름, 확장자 또는 특정 파일을 지정하세요.',
            protectHelper2: './로 시작하는 상대 경로를 사용하여 특정 파일을 보호하세요.',
            enableHelper:
