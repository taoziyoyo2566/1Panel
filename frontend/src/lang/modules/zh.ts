...
const message = {
    ...
    xpack: {
        ...
        customApp: {
            ...
            imagePrefix: '镜像地址前缀',
            imagePrefixHelper:
                '镜像地址前缀作用是: 修改 docker-compose 中 image 地址前缀，例如：设置为 1panel/repo/，则 halo 的 image 字段会从 halohub/halo-pro:2.19.3 变为 1panel/repo/halo-pro:2.19.3',
            closeHelper: '是否取消使用自定义应用商店包',
            appStoreUrlHelper: '应用商店包仅支持 .tar.gz 格式',
            postNode: '同步到子节点',
            postNodeHelper: '选择之后，子节点也使用相同的应用商店包',
            nodes: '选择节点',
        },
        alert: {
            alert: '短信告警',
